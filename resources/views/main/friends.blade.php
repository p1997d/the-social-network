                ...
                <div class="card-header">
                    ...
                    @if ($listOnline)
                        <a href="{{ route('friends', ['section' => 'online', 'id' => request('id')]) }}"
                            class="btn btn-emphasis @if ($section == 'online') active @endif">
                            Друзья онлайн
                    ...
                    @if ($user_profile == auth()->user())
                        @if ($listOutgoing->count() > 0)
                            <a href="{{ route('friends', ['section' => 'outgoing', 'id' => request('id')]) }}"
                                class="btn btn-emphasis @if ($section == 'outgoing') active @endif">
                                Исходящие заявки
                        ...
                        @if ($listIncoming->count() > 0)
                            <a href="{{ route('friends', ['section' => 'incoming', 'id' => request('id')]) }}"
                                class="btn btn-emphasis @if ($section == 'incoming') active @endif">
                                Входящие заявки
                                ...
                                <div class="position-relative">
                                    <a href="{{ route('profile', $friend->id) }}">
                                        ...
                                        @if ($friend->online()['status'])
                                            ...
                                            @else
                                                <span
                                                    class="onlineBadge position-absolute bg-body rounded-circle p-1 lh-1 text-success">
